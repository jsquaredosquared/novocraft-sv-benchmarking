...
rule split_vcf_into_category:
    ...
    output:
        vcf_file=temp(
            "../../outputs/{caller}/{sample}.{aligner}.{caller}.{category}.vcf.gz"
        ),
        index_file=temp(
            "../../outputs/{caller}/{sample}.{aligner}.{caller}.{category}.vcf.gz.tbi"
        ),
    conda:
        "../envs/vembrane.yaml"
    log:
...
rule compare_to_truth_set:
    input:
        multiext(
            "../../outputs/{caller}/{sample}.{aligner}.{caller}.{category}.vcf",
            ".gz",
            ".gz.tbi",
        ),
    output:
    ...
    params:
        truth_set=lambda wildcards: TRUTH_SETS[wildcards.sample].replace(
            ".vcf", f".{wildcards.category}.vcf"
        ),
